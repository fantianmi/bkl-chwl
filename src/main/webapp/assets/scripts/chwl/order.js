...
function wxorderSubmit(){
	var seller=$("#seller").val();
	var price=$("#price").val();
	var coinRate=$("#coinRate").val();
	if(!checkNotNull(payway)&&!checkNotNull(price)){
		swal("错误", "请确认表单是否填写完全", "error");
		return;
	}
	var url="/order/addOrder?random="+Math.round(Math.random()*100);
	var param={seller:seller,price:price,coinRate:coinRate};
	$.post(url,param,function(res){
		if(res){
			if(res.ret==0){
				swal({
				...
				var orderId=res.data["orderId"];
				var redirectURI="payprocess_wx.jsp?orderId="+orderId;
				window.location.href=redirectURI;
			}else if(res.ret==715){
				swal({
                    title: "错误",  
